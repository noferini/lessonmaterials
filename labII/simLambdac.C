/*
  Delta++ e K0s width reduced for lambda_c studies

  gaussian widths to simulate resolution effect on lambda_c
*/
...
int main(){
  ...
  Int_t npion = npartPerY[0]*2*2/100;
  Int_t nkaon = npartPerY[1]*2*2/100;
  Int_t nproton = npartPerY[2]*2*2/100;
  Int_t nk0star = npartPerY[3]*2*2/100;
  Int_t nphi = npartPerY[4]*2/100;
  Int_t ndelta = npartPerY[5]*2*2/100; // not measured
  Float_t nlambdac = npartPerY[6]*2*2/10; // not measured
  Int_t n=0;
  ...
  for(ev=0;ev < nevents;ev++){ // event loop
    ...
    // pions
    n = npion;
    if(gRandom->Rndm() < (npion-int(npion))) n++;
    for(Int_t j=0;j < n;j++){
    ...
    // kaons
    n = nkaon;
    if(gRandom->Rndm() < (nkaon-int(nkaon))) n++;
    for(Int_t j=0;j < n;j++){
    ...
    // protons
    n = nproton;
    if(gRandom->Rndm() < (nproton-int(nproton))) n++;
    for(Int_t j=0;j < n;j++){
    ...
    // K0*
    n = nk0star;
    if(gRandom->Rndm() < (nk0star-int(nk0star))) n++;
    for(Int_t j=0;j < n;j++){
      mother = -1;
    ...
    // Phi
    n = nphi;
    if(gRandom->Rndm() < (nphi-int(nphi))) n++;
    for(Int_t j=0;j < n;j++){
      mother = -1;
    ...
    // Delta
    n = ndelta;
    if(gRandom->Rndm() < (ndelta-int(ndelta))) n++;
    for(Int_t j=0;j < n;j++){
      mother = -1;
    ...
    // Lambdac
    n = nlambdac;
    if(gRandom->Rndm() < (nlambdac-int(nlambdac))) n++;
    for(Int_t j=0;j < n;j++){
      mother = -1;
